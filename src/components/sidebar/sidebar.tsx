...
function Sidebar() {
    ...
    return (
        <div ref={modalRef} className={`md:sticky top-0 left-0 h-screen w-0 md:p-4 duration-500 ${open ? "sm:w-[280px]": "sm:w-[100px]"}`}>
            ...
            <div className={`flex flex-col justify-between md:h-full bg-white dark:bg-dark-bg-secondary border border-gray-500/[0.1] dark:border-gray-500/[0.2] h-[100vh] md:rounded-[20px] md:sticky fixed md:top-0 top-0 py-4 px-4 right-0 overflow-y-auto overflow-x-hidden z-[5] transition-all duration-700 ${open ? "sm:w-full w-[280px] translate-x-[0px] opacity-[1]": "sm:w-[70px] translate-x-[400px] md:translate-x-[0px] md:opacity-[1] opacity-[0]"}`}>  
                <h1 className="flex items-center mb-2 sm:p-1">
                                ...
                                if (link.label === 'Logout') {
                                    return (
                                        <button
                                            ...
                                            className={`relative w-full text-left flex items-center justify-between px-3 py-1 h-[32px] md:rounded-[6px] duration-300 font-medium ${pathname.includes(link.link) ? "bg-bg-gray-100 dark:bg-dark-bg border border-gray-500/[0.1] dark:border-gray-500/[0.2]" : " hover:bg-gray-100 dark:hover:bg-dark-bg"}`}>
                                            <div className="flex items-center gap-2">
                                                <span className="w-[30px] opacity-[0.6]">{link.icon}</span>
                                                <span className={`flex-1 py-1 break-normal duration-500 ${open ? "" : "sm:hidden"}`}>{link.label} </span>
                                            </div>
